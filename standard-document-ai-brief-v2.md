...
**Phạm vi:** kiến trúc, mô hình AI, luồng xử lý, dữ liệu, human-in-the-loop, bảo mật, vận hành, MLOps/DocOps và cách lựa chọn Google Cloud Document AI, Azure AI Document Intelligence hoặc AWS Textract/Bedrock Data Automation, kèm lưu ý tuân thủ cho bối cảnh Việt Nam.
...
| --- | --- |
| Phiên bản | 2.0 |
| Trạng thái | Bản thảo chờ rà soát |
| Ngày cập nhật | 17/07/2026 |
| Chủ sở hữu tài liệu | _(cần bổ sung)_ |
| Chu kỳ rà soát | 6 tháng, hoặc khi nhà cung cấp có thay đổi lớn về sản phẩm/giá |
| Thay đổi chính so với v1 | Chuẩn hóa cấu trúc heading và đánh số mục; thống nhất văn phong; bổ sung mục lục, khối kiểm soát tài liệu và mục 11.3 (bối cảnh Việt Nam); rà soát trích dẫn và định dạng số |
...
Google mô tả Document AI theo mô hình các **processor** cho OCR, phân loại, tách và trích xuất; Azure cung cấp Read, Layout, prebuilt, custom extraction, classifier và composed model; AWS Textract cung cấp các API nhận diện text, biểu mẫu, bảng, query, hóa đơn, ID và lending, đồng thời AWS bổ sung Bedrock Data Automation cho pipeline IDP có generative AI. ([Google Cloud][1], [Microsoft Learn][5], [AWS Documentation][14])
...
Google Document AI trả về đối tượng `Document` có text, entities, pages, page anchors, vị trí hình học và normalized values; Azure trả về text, lines, words, polygons, tables, fields và confidence; AWS Textract biểu diễn kết quả bằng các `Block` có nội dung, quan hệ, trang và vị trí. ([Google Cloud Documentation][2], [Microsoft Learn][5], [AWS Documentation][14])
...
Sai lầm phổ biến là gom cả ba tầng vào một prompt LLM duy nhất. Cách làm này thường cho kết quả đẹp trong demo nhưng khó kiểm soát, kiểm thử và audit trong production.
...
Không ghi đè file gốc trong bất kỳ trường hợp nào: mọi biến đổi phải tạo ra bản dẫn xuất mới kèm metadata, để bảo toàn khả năng kiểm toán và tái xử lý.
...
Luôn có lớp `unknown` hoặc quy tắc từ chối. Ép mọi tài liệu vào một lớp đã biết sẽ khiến hệ thống trả về kết quả sai với độ tự tin cao — loại lỗi khó phát hiện và tốn kém nhất khi vận hành.
...
Google có custom splitter cho composite document; Azure classifier có thể nhận diện nhiều tài liệu hoặc nhiều instance trong cùng một file và trả về page range; Azure yêu cầu bật `splitMode=auto` nếu muốn tự động tách ở API v4. ([Google Cloud][1], [Microsoft Learn][10])
...
Azure custom model hỗ trợ extraction cho tài liệu đặc thù và có thể compose nhiều custom model; Google có custom extractor; AWS Textract Custom Queries dùng adapter để điều chỉnh output của mô hình query theo tài liệu doanh nghiệp. ([Microsoft Learn][5], [Google Cloud][1], [AWS Documentation][9])
...
Google Custom Extractor hỗ trợ zero-shot, few-shot và fine-tuning; tài liệu Google đề xuất khoảng 5–10 tài liệu cho few-shot và 10–50+ cho fine-tuning, tùy độ phức tạp. Đây là mức khởi động để thử nghiệm; chất lượng production phải được chứng minh bằng đánh giá trên dữ liệu thực tế của doanh nghiệp. ([Google Cloud Documentation][6])
...
Trọng số nên được hiệu chỉnh định kỳ dựa trên dữ liệu correction thực tế từ human review, thay vì cố định theo phán đoán ban đầu.
...
Human review không phải là dấu hiệu thất bại của AI, mà là một thành phần kiểm soát rủi ro bắt buộc trong thiết kế hệ thống.
...
* SLA và priority.
* Ẩn hoặc mask thông tin không cần thiết cho tác vụ review (data minimization).
* Audit log đầy đủ.
...
Nếu không lưu version, sẽ không thể giải thích vì sao cùng một tài liệu cho ra hai kết quả khác nhau ở hai thời điểm — một yêu cầu gần như bắt buộc trong kiểm toán và điều tra sự cố.
...
Azure cho biết dữ liệu và kết quả phân tích được lưu tạm thời trong cùng region và mặc định xóa sau 24 giờ, với API v4 có thể yêu cầu xóa kết quả sớm hơn. Đây là chi tiết cần đưa vào DPIA và thiết kế retention, thay vì giả định dữ liệu bị xóa ngay sau khi gọi API. ([Microsoft Learn][13])

### 11.3 Lưu ý cho bối cảnh Việt Nam

Nếu hệ thống xử lý tài liệu chứa dữ liệu cá nhân của chủ thể tại Việt Nam, cần rà soát cùng bộ phận pháp chế các điểm sau (danh sách mang tính định hướng, không thay thế tư vấn pháp lý):

* Luật Bảo vệ dữ liệu cá nhân (hiệu lực từ 01/01/2026) và trước đó là Nghị định 13/2023/NĐ-CP: cơ sở xử lý, đánh giá tác động, quyền của chủ thể dữ liệu và điều kiện chuyển dữ liệu ra nước ngoài — ảnh hưởng trực tiếp đến việc gọi API Document AI đặt tại region ngoài Việt Nam.
* Yêu cầu lưu trữ dữ liệu tại Việt Nam theo Luật An ninh mạng và Nghị định 53/2022/NĐ-CP đối với một số loại dữ liệu và doanh nghiệp thuộc phạm vi điều chỉnh.
* Hóa đơn điện tử theo Nghị định 123/2020/NĐ-CP và Thông tư 78/2021/TT-BTC đã có định dạng XML có cấu trúc: với nguồn này nên ưu tiên parse XML thay vì OCR bản thể hiện PDF, và chỉ dùng Document AI cho hóa đơn giấy hoặc bản scan.
* Chất lượng OCR tiếng Việt (đặc biệt chữ có dấu và chữ viết tay) khác nhau đáng kể giữa các nhà cung cấp; tập benchmark ở Giai đoạn 1 cần bao gồm tài liệu tiếng Việt đại diện.
...
Các khả năng trong bảng dựa trên tài liệu sản phẩm chính thức của ba nhà cung cấp. ([Google Cloud][1], [Microsoft Learn][5], [AWS Documentation][14])

### 12.1 Lựa chọn thực tế
...
Azure hiện hỗ trợ classifier trên PDF, ảnh và một số định dạng Office; custom classifier yêu cầu ít nhất hai lớp và năm mẫu cho mỗi lớp. Tài liệu Microsoft cũng khuyên thêm dữ liệu khi các lớp gần giống nhau và tạo lớp `other` cho tài liệu ngoài phạm vi. ([Microsoft Learn][10])
...
### 13.1 Mua hoặc dùng managed service cho
...
Không benchmark chỉ bằng một số ít PDF chất lượng cao được chọn lọc. Tập benchmark phải bao gồm ảnh mờ, template hiếm, tài liệu thiếu trang và những trường hợp khó mà đội vận hành gặp thường xuyên.

### 15.3 Giai đoạn 2: MVP
...
### 15.4 Giai đoạn 3: Production hardening
...
Thiếu các thành phần governance và evaluation ở cuối công thức, hệ thống vẫn có thể chạy demo tốt; nhưng khi vào production, nó sẽ nhanh chóng quay về mô hình xử lý thủ công với cả chi phí vận hành lẫn chi phí cloud cộng dồn.

[1]: https://cloud.google.com/document-ai "Document AI | Google Cloud"
[2]: https://docs.cloud.google.com/document-ai/docs/reference/rest/v1/Document "Document AI | Google Cloud Documentation"
[3]: https://docs.cloud.google.com/document-ai/docs/send-request "Send a processing request | Document AI"
[4]: https://docs.aws.amazon.com/solutions/intelligent-document-processing-on-aws/ "Intelligent Document Processing on AWS | AWS Documentation"
...
[7]: https://docs.cloud.google.com/document-ai/docs/ce-derived-signature "Custom extractor with generative AI | Document AI"
[8]: https://learn.microsoft.com/en-us/azure/ai-services/document-intelligence/concept/accuracy-confidence?view=doc-intel-4.0.0 "Interpret and improve model accuracy and confidence scores"
[9]: https://docs.aws.amazon.com/textract/latest/dg/textract-evaluating-improving-adapters.html "Evaluating and improving your adapters - Amazon Textract"
...
[11]: https://docs.aws.amazon.com/textract/latest/dg/textract-monitoring.html "Monitoring Amazon Textract - AWS Documentation"
[12]: https://docs.cloud.google.com/document-ai/docs/security "Document AI security and compliance"
[13]: https://learn.microsoft.com/en-us/azure/ai-services/document-intelligence/faq?view=doc-intel-4.0.0 "Azure AI Document Intelligence FAQ | Microsoft Learn"
