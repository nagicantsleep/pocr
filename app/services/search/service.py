...
class SearchService:
    ...
    async def search(
        ...
        Returns (results, effective_mode, degraded).
        """
        degraded = False
        if mode in ("semantic", "hybrid") and self._embed.is_mock:
            logger.warning(
                "Embedding service is in mock mode; falling back from %s to keyword search",
                mode,
            )
            mode = "keyword"
            degraded = True
